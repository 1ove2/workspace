#ifndef _CX00_INSTRUMENT_H_
#define _CX00_INSTRUMENT_H_
...
class CSx00Instrument : public QtIInstrument
{
    Q_OBJECT
    public:
        CSx00Instrument(const QHostAddress &ip, QObject *parent = 0);
        CSx00Instrument(const QString comName, uint32_t comBaud, QObject *parent = 0);
    #ifdef _PSS_GPIB_SUPPORT_
        CSx00Instrument(uint8_t cardAddr, uint8_t instrumentAddr, QObject *parent = 0);
    #endif
        ~CSx00Instrument(void);

        /* Cx00 SCPI接口 */
        void SetChannelID(uint8_t chanID);
        int SetChannelID(uint8_t chanID, uint8_t childChanID);
        ...
        int SetScanType(uint8_t chanID, bool isV, CSx00ScpiClient::CSx00ScpiSweepType isSwe);       /* 设置扫描类型 */
        ...
        int SetOutputDelay(uint8_t chanID, uint32_t delayUs);                                       /* 设置输出延时 us */
        ...
        int GPIBConfigQuery(uint8_t& devAddr, uint32_t timeout = mcWait4RecvTimeout);               /* GPIB地址请求 */
        int SetUARTConfig(uint32_t baud);                                                           /* 设置串口波特率 */
        int UARTConfigQuery(uint32_t& baud, uint32_t timeout = mcWait4RecvTimeout);                 /* 串口波特率请求 */
        int SetStartOutputEvent(uint8_t chanID, int inputLine, int outputLine, CSx00ScpiClient::CSx00ScpiTrigEdgeType trigType);/* 设置开始输出事件 */
        int SetFinishOutputEvent(uint8_t chanID, int inputLine, int outputLine, CSx00ScpiClient::CSx00ScpiTrigEdgeType trigType);/* 设置完成输出事件 */
        int SetStartSamplingEvent(uint8_t chanID, int inputLine, int outputLine, CSx00ScpiClient::CSx00ScpiTrigEdgeType trigType);/* 设置开始采样事件 */
        int SetFinishSamplingEvent(uint8_t chanID, int inputLine, int outputLine, CSx00ScpiClient::CSx00ScpiTrigEdgeType trigType);/* 设置完成采样事件 */
        int SetStartSweEvent(uint8_t chanID, int inputLine, int outputLine, CSx00ScpiClient::CSx00ScpiTrigEdgeType trigType);/* 设置开始扫描事件 */
        int ClearEvent(uint8_t chanID);                                                             /* 清除事件配置 */
        int SetTrigCount(uint8_t chanID, int count);
        ...
        int ReadAllOutput(QVector<double> &vecV, QVector<double> &vecI, uint32_t timeout = mcWait4RecvTimeout);
    ...
    private:
        ...
        CSx00ScpiClient *mScpiClient;
    
        const static int mcScpiTcpServerPort = 5025;
        static const QString mcName;
        const static int mcWait4RecvTimeout = 10000;                                                 /* 默认接收等待超时时间 */
        uint8_t mChanID;
};
...
#endif // _PSS_QT_CX00_INSTRUMENT_H_
