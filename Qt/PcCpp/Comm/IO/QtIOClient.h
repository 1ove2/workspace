...
/*---------------------------------- 预处理区 ---------------------------------*/
#ifndef _QT_QTIOCLIENT_H_
#define _QT_QTIOCLIENT_H_
...
class QtIOClient : public QThread
{
    Q_OBJECT

    public:
        QtIOClient(const QHostAddress &serverIp, uint16_t serverPort, const QString& clientName, int localPort = -1, QObject* parent = 0);
        ~QtIOClient(void);
    ...
    private slots:
        ...
        void slotClientDataReady();
        void slotClientBytesWritten(qint64 bytes);
        void slotClientErr(QAbstractSocket::SocketError socketError);
    ...
    private:
        ...
        QTcpSocket          mClient;
        static const int    mcConnectTimeoutMs = 5000;                                                      /* 连接超时 */
};
