...
/********************************** 函数实现区 *********************************/
static u8 _shb(u32 x, u32 n)
{
    return ( (x >> (n & 31)) & 0xff );
}

static u32 _shw(u32 x, u32 n)
{
    return ( (x << (n & 31)) & 0xffffffff );
}

static u32 _r(u32 x, u8 n)
{
    return ( (x >> n) | _shw(x, 32 - n) );
}

static u32 _Ch(u32 x, u32 y, u32 z)
{
    return ( (x & y) ^ ((~x) & z) );
}

static u32 _Ma(u32 x, u32 y, u32 z)
{
    return ( (x & y) ^ (x & z) ^ (y & z) );
}

static u32 _S0(u32 x)
{
    return ( _r(x, 2) ^ _r(x, 13) ^ _r(x, 22) );
}

static u32 _S1(u32 x)
{
    return ( _r(x, 6) ^ _r(x, 11) ^ _r(x, 25) );
}

static u32 _G0(u32 x)
{
    return ( _r(x, 7) ^ _r(x, 18) ^ (x >> 3) );
}

static u32 _G1(u32 x)
{
    return ( _r(x, 17) ^ _r(x, 19) ^ (x >> 10) );
}

static u32 _word(u8 *c)
{
    return ( _shw(c[0], 24) | _shw(c[1], 16) | _shw(c[2], 8) | (c[3]) );
}

static void  _addbits(SHA256_CONTEXT_T *ctx, u32 n)
{
...
{
    register u32 a, b, c, d, e, f, g, h, i;
    u32 t[2];
    u32 W[64];
    ...
    for (i = 0; i < 64; i++)
    {
        ...
        t[0] = h + _S1(e) + _Ch(e, f, g) + sK[i] + W[i];
        t[1] = _S0(a) + _Ma(a, b, c);
        h = g;
        g = f;
        f = e;
...
static void Init(SHA256_CONTEXT_T *ctx)
{
    if ( ctx != NULL )
    {
...
static void Hash(SHA256_CONTEXT_T *ctx, const void *data, i32 len)
{
    register i32 i;
...
static void Done(SHA256_CONTEXT_T *ctx, u8 *hash)
{
    register u32 i, j;
